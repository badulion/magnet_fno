...
from .dataitem import DataItem, AugmentedDataItem
...
class B1InputSARtarget(BaseTransform):
    def __init__(self):
        super().__init__()

    def _calculate_b1_plus(self, simulation: DataItem) -> npt.ArrayLike:
        """
        Method for calculating the B1 plus from the field.
        Parameters
        ----------
        simulation : DataItem
            DataItem object with the simulation data
    ...
    def _calculate_anatomical_mri(self, simulation: DataItem) -> npt.ArrayLike:
        """
        Method for calculating the anatomical MRI data.
        Parameters
        ----------
        simulation : DataItem
            DataItem object with the simulation data
        ...
        npt.ArrayLike
            anatomical MRI data
        """
        permittivity = simulation.input[1,:,:,:]
        permittivity = (permittivity - np.min(permittivity))/(np.max(permittivity) - np.min(permittivity))

        return np.expand_dims(permittivity, axis=0)
    
    def _calculate_pointwise_sar(self, simulation: DataItem) -> npt.ArrayLike:
        """
        Compute the pointwise SAR values from the E field values (without averaging), according to the formula:
    ...
    def __call__(self, simulation: DataItem):
            ...
            anatomical_mri = self._calculate_anatomical_mri(simulation)
...
class Downsample(BaseTransform):
    """
    Class for downsampling the data by taking only the second voxel in each direction.
    """
    ...
    def __call__(self, simulation: DataItem):
        """
        Downsample the data by taking only the second voxel in each direction.
        Parameters
        ----------
        data : DataItem
            DataItem object with the simulation data
        ...
        return DataItem(
            input=self._downsample_array(simulation.input),
            subject=self._downsample_array(simulation.subject),
            simulation=simulation.simulation,
            field=self._downsample_array(simulation.field),
            phase=simulation.phase,
            mask=simulation.mask,
            coils=self._downsample_array(simulation.coils),
            dtype=simulation.dtype,
            truncation_coefficients=simulation.truncation_coefficients,
        )
    
    def _downsample_array(self, array: npt.NDArray[np.float32]) -> npt.NDArray[np.float32]:
        return array[...,::2,::2,::2]
