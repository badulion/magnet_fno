...
class DiffFilterFactory:
    ...
    def curl(self) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: A tensor containing the finite difference coefficients for 
            computing the curl of a 3-dimensional vector field.
        """
        curl_filter = torch.zeros((3,3,3,3,3), dtype=torch.float32)

        # dVz / dy - dVy / dz
        curl_filter[0,2,1,0,1] = -1 / (2 * self.dx) # dy
        curl_filter[0,2,1,2,1] = 1 / (2 * self.dx)  # dy
        curl_filter[0,1,1,1,0] = 1 / (2 * self.dx)  # dz
        curl_filter[0,1,1,1,2] = -1 / (2 * self.dx) # dz

        # dVx / dz - dVz / dx
        curl_filter[1,0,1,1,0] = -1 / (2 * self.dx) # dz
        curl_filter[1,0,1,1,2] = 1 / (2 * self.dx)  # dz
        curl_filter[1,2,0,1,1] = 1 / (2 * self.dx)  # dx
        curl_filter[1,2,2,1,1] = -1 / (2 * self.dx) # dx

        # dVy / dx - dVx / dy
        curl_filter[2,1,0,1,1] = -1 / (2 * self.dx) # dx
        curl_filter[2,1,2,1,1] = 1 / (2 * self.dx)  # dx
        curl_filter[2,0,1,0,1] = 1 / (2 * self.dx)  # dy
        curl_filter[2,0,1,2,1] = -1 / (2 * self.dx) # dy
