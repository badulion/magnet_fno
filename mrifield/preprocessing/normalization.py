...
target_normalizer = StandardNormalizerSqrt()

input_normalizer.fit_params(Iterator(TRAIN_DIR), key='input', axis=0)
input_normalizer.save_as_json(f"{TRAIN_DIR}/normalization/std/input_normalization.json")

target_normalizer.fit_params(Iterator(TRAIN_DIR), key='target', axis=0)
target_normalizer.save_as_json(f"{TRAIN_DIR}/normalization/std/target_normalization.json")
